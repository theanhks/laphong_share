...
switch ($plus) {
	case 'deleteItem':
		...
		if($oId) {
			$users->update(array('status' => 2), "id = '$oId'");
	...
	case 'deleteItems':
		...
		if($oIds) {
			$users->update(array('status' => 2), "id IN (".join(',',$oIds).")");
			$infoText = $amessages['item_delete_ok'];
			$infoClass = "infoText";
		} else {
	...
	case 'activateItem':
		$oId = $request->element('oId');
		if($oId) {
			$users->update(array('status' => 1), "id = '$oId'");
	...
	case 'activateItems':
		$oIds = $request->element('oIds');
		if($oIds) {		
			$users->update(array('status' => 1), "id IN (".join(',',$oIds).")");
			$infoText = $amessages['item_activate_ok'];
			$infoClass = "infoText";
		} else {
	...
	case 'disableItem':
		$oId = $request->element('oId');
		if($oId) {
			$users->update(array('status' => 0), "id = '$oId'");
	...
	case 'disableItems':
		$oIds = $request->element('oIds');
		if($oIds) {		
			$users->update(array('status' => 0), "id IN (".join(',',$oIds).")");
			$infoText = $amessages['item_disable_ok'];
			$infoClass = "infoText";
		} else {
			$infoText = $amessages['disable_error_no_item'];
...
$searchCond = "status!=3";			# For other cases, you can set this to "(1>0)"
if($searchStatus != '-1') $searchCond .= " AND status = '$searchStatus'";
if($searchTerms) $searchCond .= " AND (username LIKE '%".$searchTerms."%' OR fname LIKE '%".$searchTerms."%' OR lname LIKE '%".$searchTerms."%' OR id = '".$searchTerms."')";
if($searchUserType!='-1') $searchCond .= " AND type= '$searchUserType'";
# Get number of rows, pages
$pages = $users->getNumItems('id', $searchCond, $items_per_pages);
$rows = $pages['rows'];
$pages = $pages['pages'];
if($page > $pages) $page = $pages;

# Get all categories object and pass to template
$usersItems = $users->getObjects($page, $searchCond, array($orderBy => $orderDir), $items_per_pages);
$template->assign("listObjects",$usersItems);

$listUserType = $usertypes->comboListObjects('id','vn_type');
$template->assign("listUserType",$listUserType);

# Generate and pass link pager to template
$listPage = linkPager(ADMIN_SCRIPT."?op=admin&part=user&act=list&amp;searchTerms=".$searchTerms."&amp;status=".$searchStatus."&amp;orderBy=".$orderBy."&amp;orderDir=".$orderDir."&amp;page=%s",$pages,$page);
