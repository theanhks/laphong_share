...
switch ($plus) {
	case 'deleteItem':
		...
		if($oId) {
			$pro_cate->update(array('status' => 2), "id = '$oId'");
	...
	case 'deleteItems':
		...
		if($oIds) {
			$pro_cate->update(array('status' => 2), "id IN (".join(',',$oIds).")");
			$infoText = $amessages['item_delete_ok'];
			$infoClass = "infoText";
		} else {
	...
	case 'activateItem':
		$oId = $request->element('oId');
		if($oId) {
			$pro_cate->update(array('status' => 1), "id = '$oId'");
	...
	case 'activateItems':
		$oIds = $request->element('oIds');
		if($oIds) {		
			$pro_cate->update(array('status' => 1), "id IN (".join(',',$oIds).")");
			$infoText = $amessages['item_activate_ok'];
			$infoClass = "infoText";
		} else {
	...
	case 'disableItem':
		$oId = $request->element('oId');
		if($oId) {
			$pro_cate->update(array('status' => 0), "id = '$oId'");
	...
	case 'disableItems':
		$oIds = $request->element('oIds');
		if($oIds) {		
			$pro_cate->update(array('status' => 0), "id IN (".join(',',$oIds).")");
			$infoText = $amessages['item_disable_ok'];
			$infoClass = "infoText";
		} else {
			$infoText = $amessages['disable_error_no_item'];
	...
	case 'changePosition':
		$oId = $request->element('oId');
		$position = $request->element('position');
		if($pro_cate->changePosition($oId, $position)) {
...
$searchCond = "1>0 and status <> 0";	
# For other cases, you can set this to "(1>0)"
...
$pages = $pro_cate->getNumItems('id', $searchCond, $items_per_pages);
$rows = $pages['rows'];
$pages = $pages['pages'];
if($page > $pages) $page = $pages;

# Get all categories object and pass to template
$categoryItems = $pro_cate->getObjects($page, $searchCond, array($orderBy => $orderDir), $items_per_pages);
