...
switch ($plus) {
	case 'deleteItem':
		...
		if($oId) {
			$members->update(array('status' => 2), "id = '$oId'");
	...
	case 'deleteItems':
		...
		if($oIds) {
			$members->update(array('status' => 2), "id IN (".join(',',$oIds).")");
			$infoText = $amessages['item_delete_ok'];
			$infoClass = "infoText";
		} else {
	...
	case 'activateItem':
		$oId = $request->element('oId');
		if($oId) {
			$members->update(array('status' => 1), "id = '$oId'");
	...
	case 'activateItems':
		$oIds = $request->element('oIds');
		if($oIds) {		
			$members->update(array('status' => 1), "id IN (".join(',',$oIds).")");
			$infoText = $amessages['item_activate_ok'];
			$infoClass = "infoText";
		} else {
	...
	case 'disableItem':
		$oId = $request->element('oId');
		if($oId) {
			$members->update(array('status' => 0), "id = '$oId'");
	...
	case 'disableItems':
		$oIds = $request->element('oIds');
		if($oIds) {		
			$members->update(array('status' => 0), "id IN (".join(',',$oIds).")");
			$infoText = $amessages['item_disable_ok'];
			$infoClass = "infoText";
		} else {
			$infoText = $amessages['disable_error_no_item'];
...
if($searchTerms) $searchCond .= " AND (username LIKE '%".$searchTerms."%' OR fname LIKE '%".$searchTerms."%' OR lname LIKE '%".$searchTerms."%' OR email LIKE '%".$searchTerms."%')";

# Get number of rows, pages
$pages = $members->getNumItems('id', $searchCond, $items_per_pages);
$rows = $pages['rows'];
$pages = $pages['pages'];
if($page > $pages) $page = $pages;

# Get all categories object and pass to template
$membersItems = $members->getObjects($page, $searchCond, array($orderBy => $orderDir), $items_per_pages);
$template->assign("listObjects",$membersItems);

# Generate and pass link pager to template
$listPage = linkPager(ADMIN_SCRIPT."?op=manage&part=member&act=list&amp;searchTerms=".$searchTerms."&amp;status=".$searchStatus."&amp;orderBy=".$orderBy."&amp;orderDir=".$orderDir."&amp;page=%s",$pages,$page);
