...
$templateFile = "manageeditimgproduct.temp.html";
...
$searchAlbum = $request->element('searchAlbum',0);
$searchProduct = $request->element('searchProduct',0);
$album = 1;
$product = 1;
...
$sp = $imgproducts->getRecord($aId);
if ($aId && $sp != ''){
$imgItem = $imgproducts->getObject($aId);
$template->assign("listObject",$imgItem);
if($_POST) {
		# Validation
		...
		$files = isset($_FILES['file1'])?$_FILES['file1']:'';
			...
			}else $file_name=$imgItem->getFile1();
			$file1 = $file_name;
			...
			$imgproducts->updateData($fields,$aId);
			...
			$template->assign("searchProduct",$searchProduct);
		}else {
...
$listProducts = $products->comboListObjects('id','vn_name',$searchProduct);
$template->assign("listProducts",$listProducts);
$listAlbumpro = $albumproducts->comboListObjects('id','vn_name',$searchAlbum);
