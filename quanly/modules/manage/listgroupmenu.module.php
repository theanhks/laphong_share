...
switch ($plus) {
	case 'deleteItem':
		...
		if($oId) {
			$mg->update(array('status' => 2), "id = '$oId'");
	...
	case 'deleteItems':
		...
		if($oIds) {
			$mg->update(array('status' => 2), "id IN (".join(',',$oIds).")");
			$infoText = $amessages['item_delete_ok'];
			$infoClass = "infoText";
		} else {
			$infoText = $amessages['delete_error_no_item'];
	...
	case 'activateItem':
		$oId = $request->element('oId');
		if($oId) {
			$mg->update(array('status' => 1), "id = '$oId'");
	...
	case 'activateItems':
		$oIds = $request->element('oIds');
		if($oIds) {		
			$mg->update(array('status' => 1), "id IN (".join(',',$oIds).")");
			$infoText = $amessages['item_activate_ok'];
			$infoClass = "infoText";
		} else {
	...
	case 'disableItem':
		$oId = $request->element('oId');
		if($oId) {
			$mg->update(array('status' => 0), "id = '$oId'");
	...
	case 'disableItems':
		$oIds = $request->element('oIds');
		if($oIds) {		
			$mg->update(array('status' => 0), "id IN (".join(',',$oIds).")");
			$infoText = $amessages['item_disable_ok'];
			$infoClass = "infoText";
		} else {
...
$pages = $mg->getNumItems('id', $searchCond, $items_per_pages);
$rows = $pages['rows'];
$pages = $pages['pages'];
if($page > $pages) $page = $pages;

# Get all categories object and pass to template
$categoryItems = $mg->getObjects($page, $searchCond, array($orderBy => $orderDir), $items_per_pages);
$template->assign("listObjects",$categoryItems);

# Generate and pass link pager to template
$listPage = linkPager(ADMIN_SCRIPT."?op=manage&amp;part=menu&amp;act=listGroup&amp;searchTerms=".$searchTerms."&amp;status=".$searchStatus."&amp;orderBy=".$orderBy."&amp;orderDir=".$orderDir."&amp;page=%s",$pages,$page);
