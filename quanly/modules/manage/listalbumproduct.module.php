...
switch ($plus) {
	case 'deleteItem':
		...
		if($oId) {
			$albumproducts->update(array('status' => 2), "id = '$oId'");
	...
	case 'deleteItems':
		...
		if($oIds) {
			$albumproducts->update(array('status' => 2), "id IN (".join(',',$oIds).")");
			$infoText = $amessages['item_delete_ok'];
			$infoClass = "infoText";
		} else {
	...
	case 'activateItem':
		$oId = $request->element('oId');
		if($oId) {
			$albumproducts->update(array('status' => 1), "id = '$oId'");
	...
	case 'activateItems':
		$oIds = $request->element('oIds');
		if($oIds) {		
			$albumproducts->update(array('status' => 1), "id IN (".join(',',$oIds).")");
			$infoText = $amessages['item_activate_ok'];
			$infoClass = "infoText";
		} else {
	...
	case 'disableItem':
		$oId = $request->element('oId');
		if($oId) {
			$albumproducts->update(array('status' => 0), "id = '$oId'");
	...
	case 'disableItems':
		$oIds = $request->element('oIds');
		if($oIds) {		
			$albumproducts->update(array('status' => 0), "id IN (".join(',',$oIds).")");
			$infoText = $amessages['item_disable_ok'];
			$infoClass = "infoText";
		} else {
			$infoText = $amessages['disable_error_no_item'];
	...
	case 'up':
		...
		if($albumproducts->up($oId, $position)) {
	...
	case 'down':
		$oId = $request->element('oId');
		$position = $request->element('position');
		if($albumproducts->down($oId, $position)) {
	...
	case 'changePosition':
		$oId = $request->element('oId');
		$position = $request->element('position');
		if($albumproducts->changePosition($oId, $position)) {
...
if($searchTerms) $searchCond .= " AND (vn_name LIKE '%".$searchTerms."%' OR en_name LIKE '%".$searchTerms."%' OR slug LIKE '%".$searchTerms."%')";

# Get number of rows, pages
$pages = $albumproducts->getNumItems('id', $searchCond, $items_per_pages);
$rows = $pages['rows'];
$pages = $pages['pages'];
if($page > $pages) $page = $pages;

# Get all categories object and pass to template
$categoryItems = $albumproducts->getObjects($page, $searchCond, array($orderBy => $orderDir), $items_per_pages);
