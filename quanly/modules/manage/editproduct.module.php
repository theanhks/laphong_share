...
if ($Id && $sp != '') {	
	...
	$en_nsx= $request->element('en_nsx',NULL);
	...
	$file3_old=$request->element('file3_old','');
...
$template->assign("iconhot_old",$iconhot_old);
	...
	if($_POST) {
		...
		if($validate == '') {
			...
			/* upload hinh tieng anh*/
			$files3 = isset($_FILES['file3'])?$_FILES['file3']:'';
			$upload3 = new Upload($files3);
			if($files3['tmp_name']){
				/* xoa hinh cu upload hinh moi*/
				unlink(ROOT_PATH."gallery/avatar_upload/products/storage/".$file3_old);
				unlink(ROOT_PATH."gallery/avatar_upload/products/thumb/".$file3_old);
				unlink(ROOT_PATH."gallery/avatar_upload/products/avatar/".$file3_old);
				unlink(ROOT_PATH."gallery/avatar_upload/products/medium/".$file3_old);
			...
			}else $file3_name = $file3_old;	
			
			
			/* upload hinh tieng viet*/
	...
	{}*/
			...
			//var_dump($newhot);
		$productInfo = new ProductInfo($slug, $vn_name, $en_name, $vn_details, $en_details,$vn_nsx,$en_nsx,$vn_nhanhieu,$en_nhanhieu,$price, $s_price, $file_name,$photos1, $kichthuocs, $file3_name, '', '', $position, $date_published, $num_product, $category, $status,$spkhuyenmai, $newhot, $spmoi,$properties);
		...
		$field = array(
			...
			# Generate and pass link pager to template
                                               //op=manage&part=product&act=list&searchTerms=&searchStatus=1&orderBy=vn_name&orderDir=ASC
			$listPage = linkPager(ADMIN_SCRIPT."?op=manage&amp;part=product&amp;act=list&amp;searchTerms=".$searchTerms."&amp;searchStatus=".$searchStatus."&amp;orderBy=".$orderBy."&amp;orderDir=".$orderDir."&amp;page=%s",$pages,$page);
