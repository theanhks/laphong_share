...
class Validate extends DB{
	...
	function validString($name,$str) {
		if(!$str) return $name." - ".$this->messages['notValidString'].'<br />';
		return '';	
	}
	function validStatus($name,$str) {
		if($str == -1) return $name." - ".$this->messages['notValidStatus'].'<br />';
		return '';	
	}	
	function validPassword($name,$str) {
		if(strlen($str)<6) return $name." - ".$this->messages['notValidPassword'].'<br />';
		return '';	
	}
	function validTestPass($name,$password,$confirm) {
		if($password != $confirm) return $name." - ".$this->messages['notValidTestPass'].'<br />';
		return '';	
	}
		
	function validEmail($name,$email) {
		if(!$email)	return $name." - ".$this->messages['notValidString'].'<br />';
		...
		return ((preg_match($lengthPattern, $email) > 0) && (preg_match($syntaxPattern, $email) > 0)) ? '' : $name." - ".$this->messages['notValidEmail'].'<br />';
	}
	...
	function validUsername($name,$username) {
		$error = '';
		if(!$username) return $name." - ".$this->messages['notValidUsername'].'<br />';
	...
	function validSlug($name,$slug) {
		...
		function validRDateTime($name,$ryear,$rmooth,$rday) {
		$year = date("Y");
		$mooth = date("m");
		$day = date("d");
		if ($ryear < $year) {
			return $name." - ".$this->messages['notValidRDateTime'];
		}
		if (($ryear == $year) && ($rmooth < $mooth)){
			return $name." - ".$this->messages['notValidRDateTime'];
		}
		if (($ryear == $year) && ($rmooth == $mooth) && ($rday < $day)){
			return $name." - ".$this->messages['notValidRDateTime'];
		}
		return '';
	}
