...
include_once(ROOT_PATH."classes/dao/productinfoseach.class.php");
...
* Return: Array of Info objects
*-----------------------------------------------------------------------*/
	function getObjectsSearch($page = 1, $condition = '`pid` = 0', $sort = array(), $items_per_page = DEFAULT_ADMIN_ROWS_PER_PAGE) {
		if(!$page) $page = 1;
		$start = ($page -1) * $items_per_page;
		$results = $this->select("`id`,`slug`,`vn_name`,`en_name`", $condition, $sort, $start, $items_per_page);
		var_dump($results);
		if($results) {
			$productInfos = array();
			foreach($results as $key => $result) {
				$productInfos[] = new ProductInfoSearch (
											$result['slug'],
											$result['vn_name'],
											$result['en_name'],
											$result['id']
...
*-----------------------------------------------------------------------*/
	...
	function changePrice($oId = 0, $price = 0) {
		if(!$oId) return 0;
		if($this->update(array('position' => $position), "`id` = '$oId'")) return 1;
		return 0;
	}
	...
	function getProductCategoryCount($pId) {
		$result = $this->select("count(`id`)", "category='$pId'");
		if($result) {
			return $result[0][0];
		}
		return "";
