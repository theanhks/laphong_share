...
class AdsGroups_home extends Model {
	function AdsGroups_home($database = '') {
		if(!$database) {
			global $db;
			$this->_db = $db;
		} else $this->_db = $database;
		$this->table = DB_PREFIX."ad_groups_home";
	}
...
*-----------------------------------------------------------------------*/	
	...
	function getParentId($pId, $lang = 'vn') {
...
*-----------------------------------------------------------------------*/
	...
	function getClassMetroJs($gId) {
		$result = $this->select("`en_name`", "`id` = '$gId'");
		if($result) {
			return $result[0][0];
		}
		return "";
	}
	function getEffectMetroJs($gId) {
		$result = $this->select("`status`", "`id` = '$gId'");
		if($result) {
			return $result[0][0];
		}
		return "";
