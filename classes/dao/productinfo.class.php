...
class ProductInfo {
	...
	function ProductInfo($slug, $vn_name, $en_name, $vn_details, $en_details,$vn_nsx,$en_nsx,$vn_nhanhieu, $en_nhanhieu, $price, $s_price, $avatar,$file1, $file2, $file3, $file4, $file5, $position, $date_published, $num_product, $category, $status,$payment, $viewed, $home,$properties, $pId = 0)
	{
		...
		$this->vn_nsx = stripslashes($vn_nsx);
		$this->en_nsx = stripslashes($en_nsx);
		$this->vn_nhanhieu = stripslashes($vn_nhanhieu);
		$this->en_nhanhieu = stripslashes($en_nhanhieu);
		$this->price = stripslashes($price);
		$this->s_price = stripslashes($s_price);
		$this->avatar = $avatar;
		$this->file1 = $file1;
		$this->file2 = $file2;
		$this->file3 = $file3;
	...
	function getViewUrl($lang='vn') {
		...
		if(URL_TYPE == 1) {			# Query string
			$url = ROOTPATH."/index.php?op=productdetail&lang=".$lang."slug=".$this->slug."&id=".$this->pId;		
			return $url;
		} elseif(URL_TYPE == 2) {	# SEO
			if(SUB_DOMAIN) $url = ROOTPATH."/".$lang."/".$this->pId."/".$this->slug.".html";
			else $url = ROOTPATH."/".$lang."/".$this->pId."/".$this->slug.".html";
			return $url;		
		} else return "";	
	}
